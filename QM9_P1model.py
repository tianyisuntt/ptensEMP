...
class ConvolutionalLayer(torch.nn.Module):
    ...
    def forward(self, x: ptens.ptensors1, G: ptens.graph) -> ptens.ptensors1:
        ...
        x2 = self.dropout(x2)
        x2 = self.lin2(x2)
        x3 = ptens.linmaps0(x2)
        x3 = x3.torch()
        x3 = ptens.ptensors1.from_matrix(x3,atoms)
        return x3

class Model(torch.nn.Module):
    def __init__(self, in_channels: int, convolution_dim: int, dense_dim: int) -> None:
        super().__init__()
        self.conv1 = ConvolutionalLayer(in_channels,     convolution_dim, 1)
