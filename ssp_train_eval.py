...
def run(
    ...
    for i_run in range(runs):
        ...
        data = data.to(device)
        if use_ptens:
            data.G = p.graph.from_matrix(torch.sparse_coo_tensor(data.edge_index,torch.ones(data.edge_index.size(1),dtype=bool)).float().to_dense())

        model.to(device).reset_parameters()
        if str_preconditioner == 'KFAC':

            preconditioner = ssp_psgd.KFAC(
                model, 
                eps, 
                sua=False, 
                pi=False, 
