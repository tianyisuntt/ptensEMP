...
class CleanupData(BaseTransform):
  def __call__(self, data):
    ...
    data.y = data.y.float(32)#.cuda()
    return data
class ToPtens_Batch(BaseTransform):
  def __call__(self, data):
    data.G = ptens.graph.from_matrix(torch.sparse_coo_tensor(data.edge_index,
                                                             torch.ones(data.edge_index.size(1),dtype=torch.float32),
...
def compute_accuracy(loader):
  ...
  with torch.no_grad():
    ...
    for batch in loader:
      ...
      pred = model(batch.x,batch.G,batch.batch)
...
for epoch in range(epoch0 + 1,epochs):
  ...
  for batch in train_loader:
    ...
    l = loss(model(batch.x,batch.G,batch.batch)[:,target_index],batch.y[:,target_index])
    l.backward()
    optimizer.step()
    inc = l.tolist()
    ind += 1
