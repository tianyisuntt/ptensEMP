...
dataset_name = 'qm9'
best_validation_model_path = 'qm9_val_GCNmodel.pt'
model_path = 'qm9_GCNmodel.pt'
try_reload = True

class CleanupData(BaseTransform):
    def __call__(self, data):
        data.x = data.x.to('cuda')
        data.y = data.y.float().to('cuda')
        return data
...
class ConvolutionalLayer(torch.nn.Module):
    ...
    def forward(self, x: ptens.ptensors1, G: ptens.graph) -> ptens.ptensors1:
        x1 = x.transfer1(G.nhoods(self.nhops),G,True).to('cuda')
        atoms = x1.get_atoms()
        x2 = x1.torch()
    #
...
def compute_accuracy(loader):
    ...
    with torch.no_grad():
        ...
        for subset in loader:
            ...
            pred = model(batch.x,batch.G,batch.batch.to('cuda'))[:,target_index]
            preds.append(pred)
...
for epoch in range(epoch0 + 1,epochs):
    ...
    for subset in train_loader:
        batch = on_learn_transform(subset)
        optimizer.zero_grad()
        loss = criterion(model(batch.x,batch.G,batch.batch.to('cuda'))[:,target_index],batch.y[:,target_index]).to('cuda')
        loss.backward()
        optimizer.step()
        inc = loss.tolist()
        index += 1
...
val_acc = checkpoint['validation_score']
print("\tEpoch:",epoch)
