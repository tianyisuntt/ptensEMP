"""
This code primarily originates from "https://github.com/russellizadi/ssp".
"""
import argparse
import numpy as np
import torch
...
class CRD(torch.nn.Module):
    def __init__(self, d_in, d_out, p):
        ...
        self.conv = GCNConv(d_in, d_out, cached=True, bias=use_bias,normalize=use_normalization) 
        self.p = p
    ...
    def forward(self, x, edge_index, mask=None):
        ...
        x = F.leaky_relu(self.conv(x, edge_index),0.5)
...
class CRD_ptens(torch.nn.Module):
    def __init__(self, d_in, d_out, p):
        super(CRD_ptens, self).__init__()
        self.conv = pl.GCNConv(d_in, d_out, bias=use_bias) 
        self.p = p
    ...
    def forward(self, x: p.ptensors0, G: p.graph, mask=None) -> p.ptensors0:
        x = p.relu(self.conv(x,G),0.5)
        #x = F.dropout(x, p=self.p, training=self.training)
        return x
class CLS_ptens(torch.nn.Module):
    def __init__(self, d_in, d_out):
        super(CLS_ptens, self).__init__()
        self.conv = pl.GCNConv(d_in, d_out, bias=use_bias)
    ...
    def forward(self, x: p.ptensors0, G: p.graph, mask=None) -> torch.Tensor:
        x = self.conv(x,G)
        x = x.torch()
        x = F.log_softmax(x, dim=1)
        return x
...
class Net_ptens(torch.nn.Module):
    def __init__(self, dataset):
        super(Net_ptens, self).__init__()
        self.crd = CRD_ptens(dataset.num_features, args.hidden, args.dropout)
        self.cls = CLS_ptens(args.hidden, dataset.num_classes)
    ...
    def forward(self, data):
        ...
        x = self.crd(x, G, data.train_mask)
        x = self.cls(x, G, data.train_mask)
        return x
...
kwargs = {
    'dataset': dataset, 
    'model': Net_ptens(dataset) if use_ptens else Net(dataset), 
